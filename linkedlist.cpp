...
public:
	...
	void setHeadPtr(node<T>* newNode)
	{
		headPtr = newNode;
	}
	int getCount()
...
template <class T>
int addNode(LinkedList<T>* pList, int pos, T data)
{
	//check errors
...
template <class T>
int removeNode(LinkedList<T>* pList, int pos)
{
	//if empty
	if(pList->getCount() == 0)
	{
...
template <class T>
void showNode(LinkedList<T>* pList)
{
	//if empty
	if(pList->getCount() == 0)
	{
...
template <class T>
bool isEmpty(LinkedList<T>* pList)
{
	if(pList->getCount() == 0)
	{
		return true;
...
template <class T>
int findPos(LinkedList<T>* pList, T data)
{
	//if empty
	if(pList->getCount() == 0)
	{
...
template <class T>
void makeEmpty(LinkedList<T>* pList)
{
	//if empty
	if(pList->getCount() == 0)
	{
